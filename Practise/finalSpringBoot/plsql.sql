DECLARE
type fruitsList is VARRAY(7) OF VARCHAR(30);
fruits fruitsList;
total integer;
BEGIN
fruits := fruitsList('mango','banana','grapes','apple','litchy','imli');
total := fruits.count;
for a in 1..total loop
  dbms_output.put_line('fruit : '||fruits(a));
end loop;
END;
/
